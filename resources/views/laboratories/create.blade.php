...
<div class="max-w-lg">
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8">
        <div class="mb-6">
            <h2 class="text-xl font-bold text-slate-900">Tambah Laboratorium Baru</h2>
            <p class="text-sm text-slate-500 mt-1">Masukkan informasi ruangan laboratorium</p>
        </div>
        ...
        <form method="POST" action="{{ route('laboratories.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="nama_lab" id="nama_lab" value="{{ old('nama_lab') }}" required placeholder="Lab Hardware"
            ...
            <div>
                ...
                <input type="text" name="lokasi" id="lokasi" value="{{ old('lokasi') }}" required placeholder="Gedung A - Lantai 1"
                       class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-all">
            </div>
