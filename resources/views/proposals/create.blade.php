...
<div class="max-w-3xl">
    <div class="mb-6">
        <a href="{{ route('proposals.index') }}" class="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-indigo-600 transition-colors">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8 mb-6">
            ...
            <div class="space-y-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label for="nomor_surat" class="block text-sm font-medium text-slate-700 mb-1.5">Nomor Surat</label>
                        <input type="text" name="nomor_surat" id="nomor_surat" value="{{ old('nomor_surat') }}" required
                               placeholder="001/UPT-LAB/VII/2026"
                    ...
                    <div>
                        <label for="tanggal_surat" class="block text-sm font-medium text-slate-700 mb-1.5">Tanggal Surat</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="lampiran" id="lampiran" value="{{ old('lampiran', '1 (satu) lembar') }}" required
                               placeholder="1 (satu) lembar"
                               class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-all">
                    </div>
                    <div>
                        <label for="perihal" class="block text-sm font-medium text-slate-700 mb-1.5">Perihal</label>
                        <input type="text" name="perihal" id="perihal" value="{{ old('perihal', 'Permohonan Pengadaan Barang') }}" required
            ...
            <div id="items-container" class="space-y-4">
                ...
                <div class="item-card bg-slate-50 rounded-xl border border-slate-200 p-5 relative" data-index="0">
                    <div class="flex items-center justify-between mb-4">
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">Item #<span class="item-number">1</span></span>
                        <button type="button" class="remove-item-btn hidden p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all" title="Hapus Item">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="details[0][nama_barang]" required placeholder="Komputer Desktop"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        addBtn.addEventListener('click', function () {
            ...
            newItem.className = 'item-card bg-slate-50 rounded-xl border border-slate-200 p-5 relative animate-slide-down';
