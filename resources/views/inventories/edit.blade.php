...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8">
        <div class="mb-6">
            ...
            <p class="text-sm text-slate-500 mt-1">Perbarui informasi barang <span class="font-semibold text-slate-700">{{ $inventory->nama_barang }}</span></p>
        </div>
        ...
        <form method="POST" action="{{ route('inventories.update', $inventory) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="kode_barang" id="kode_barang" value="{{ old('kode_barang', $inventory->kode_barang) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="nama_barang" id="nama_barang" value="{{ old('nama_barang', $inventory->nama_barang) }}" required
                           class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <input type="number" name="jumlah" id="jumlah" value="{{ old('jumlah', $inventory->jumlah) }}" required min="0"
                           class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-all">
                </div>
                ...
                <div>
                    ...
                    <select name="laboratorium_id" id="laboratorium_id" required
                        ...
                        @foreach($laboratories as $lab)
                            <option value="{{ $lab->id }}" {{ old('laboratorium_id', $inventory->laboratorium_id) == $lab->id ? 'selected' : '' }}>
                                {{ $lab->nama_lab }}
                            </option>
                        @endforeach
                    </select>
